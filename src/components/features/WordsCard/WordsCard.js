...
import { wordsCardVocabGenerator } from "../../../scripts/vocabCardGenerator";
...
const WordsCard = ({ soundType, soundMode }) => {
  ...
  // Focus the hidden input on mount only. Without a dependency array this ran
  // after every render and stole focus back on every state change, making the
  // catalog/links unusable by keyboard and causing scroll jumps. onBlur keeps
  // focus on the input during typing.
  useEffect(() => {
  ...
  // An empty chapter or a transient out-of-range index (index and wordsDict
  // update in separate effects) previously crashed the render with
  // "Cannot read properties of undefined (reading 'key')". Compute everything
  // null-safely here; the early-return guard lives below all hooks so the
  // Rules of Hooks are not violated.
  const currentEntry = wordsDict && wordsDict[index];
  const currWord = currentEntry ? currentEntry.key : "";
  const currMeaning = currentEntry ? currentEntry.val : "";
  const extra = currInput.slice(currWord.length, currInput.length).split("");
  const chapterRange =
    DICTIONARY_SOURCE_CATALOG[vocabSource] &&
    DICTIONARY_SOURCE_CATALOG[vocabSource][currChapter];
  const currChapterStartIndex = chapterRange ? chapterRange[0] : 0;
  const currChapterEndIndex = chapterRange ? chapterRange[1] : 0;
  ...
  // Safe to bail out here — after every hook has been called.
  if (!currentEntry) {
    return (
      <div className="words-card-container">
        <div className="words-card-main">
          <div className="wordcard-meaning-display-field">Loading…</div>
        </div>
      </div>
    );
  }
