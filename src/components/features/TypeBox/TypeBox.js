...
const TypeBox = ({
  ...
  useEffect(() => {
    // Refill the word pool whenever the cursor approaches the end of the
    // current list. Previously this only fired at the exact index
    // DEFAULT_WORDS_COUNT - 1, so after the first refill it never triggered
    // again and a fast typist would run out of words mid-test (crash on
    // words[currWordIndex].split). Triggering on proximity keeps a buffer ahead.
    const REFILL_BUFFER = 10;
    if (currWordIndex >= words.length - REFILL_BUFFER) {
      if (language === ENGLISH_MODE) {
  ...
  const UpdateInput = (e) => {
    ...
    // Immutable update so React reliably re-renders consumers of this state.
  ...
  const handleKeyDown = (e) => {
    ...
    // Use functional updates so rapid keystrokes (and the extra increment in
    // checkPrev within the same event) don't clobber each other via stale
    // closures.
    if (status === "started") {
      setRawKeyStrokes((prev) => prev + 1);
      // Count every printable character (letters, digits, symbols) toward the
      // effective WPM keystrokes. `key.length === 1` is true for all single
      // printable chars; the space (keyCode 32) is excluded here because it is
      // counted on word completion in checkPrev().
      if (key && key.length === 1 && keyCode !== 32) {
  ...
  const checkPrev = () => {
    ...
    if (isCorrect) {
      ...
      // Immutable Set updates so React is notified of the change.
      setWordsCorrect((prev) => new Set(prev).add(currWordIndex));
      setWordsInCorrect((prev) => {
        const next = new Set(prev);
        next.delete(currWordIndex);
        return next;
      });
      setInputWordsHistory((prev) => ({
        ...prev,
